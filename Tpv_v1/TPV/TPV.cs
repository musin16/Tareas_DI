...
namespace TPV
{
    public partial class TPV : Form
    {
        ...
        private void btnEmpezar_Click(object sender, EventArgs e)
        {
            ...
            lblDni.Visible = true;
            lblEmailCli.Visible = true;
            txtDniCliente.Visible = true;
            txtMailCli.Visible= true;
        ...
        private void dgv1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (confirmarEliminar == DialogResult.Yes)
            {
                ...
                {
                    MessageBox.Show("No puedes eliminar el encabezado");
                }
            }
        }
        ...
        private void txtDniCliente_TextChanged(object sender, EventArgs e)
        {
            ...
            if (contClien==1)
            {
                listaClientes=conexion.encontrarCliente();
           
            }
            if (txtDniCliente.Text.Length>=5)
            {
                for (int i = 0; i < listaClientes.Count; i++)
                {
                    if (listaClientes[i].Dni == txtDniCliente.Text)
                    {
                        txtMailCli.Text = listaClientes[i].Email;
                        txtMailCli.Enabled = false;
                    }
                }
            }
      /*listCli.Find(fac => fac.Dni == txtDniCliente.Text);
            if (listCli.Find(fac => fac.Dni == txtDniCliente.Text)!=null)
            {
                t
            }*/
        }
